...
Usage:
    ...
    python verify.py deck.html --slides 10                  # Slide mode: capture each slide one by one (first 10)
...
def verify_html(html_path, viewports=None, slides=0, output_dir=None, show=False, wait=2000):
    ...
    with sync_playwright() as p:
        ...
        for viewport in viewports:
            ...
            if show:
                print("  (Browser window left open. Press Enter to close...)")
                input()
...
def main():
    ...
    parser.add_argument("--slides", type=int, default=0,
                        help="Slide mode: capture the first N slides (HTML must support ArrowRight to advance)")
    parser.add_argument("--output", default=None,
                        help="Output directory (defaults to a screenshots/ folder next to the HTML file)")
    parser.add_argument("--show", action="store_true",
                        help="Non-headless mode; opens a real browser window")
    parser.add_argument("--wait", type=int, default=2000,
                        help="Number of milliseconds to wait after the page opens (default: 2000)")
