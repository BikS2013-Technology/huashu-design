...
(async () => {
  ...
  if (hasReady) {
    // Second line of defense: actively reset the animation time to zero — handles
    // HTML that does not strictly follow the starter tick template (for example,
    // when lastTick uses performance.now(), the font-loading time gets folded
    // into the first frame's dt). See references/animation-pitfalls.md §12 for details.
    ...
    if (seekCorrected) {
      // Wait two rAFs so the seek takes effect and the t=0 frame is actually rendered
